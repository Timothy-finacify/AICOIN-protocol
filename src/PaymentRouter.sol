...
contract PaymentRouter {
    ...
    uint256 public constant TREASURY_PERCENT = 15; // 1.5% in basis points (15/1000)
    ...
    function routePayment(address company, uint256 amount) external returns (bool) {
        ...
        IAICOIN(aicoinToken).transfer(BURN_ADDRESS, burnAmount);
        IAICOIN(aicoinToken).transfer(treasury, treasuryAmount);
        IAICOIN(aicoinToken).transfer(company, companyAmount);
